...
public class Main {
    ...
    static List<Particle> repopulate(List<Particle> oldParticles) {
        ...
        for (int i = 0; i < particles_count; i++) {
            ...
            Particle p = oldParticles.get(index);
            newParticles.add(new Particle(p.x, p.y, p.w)); // Додайте нову частинку з відомими координатами та вагою
        }
    ...
    public static void move(List<Particle> particles, double x, double y) {
        particles.forEach(p -> {
            ...
            } while (Math.abs(randomX) > Sx); // Повторюємо генерацію, якщо число виходить за межі

            do {
                randomY = ThreadLocalRandom.current().nextGaussian() * Sy;
            } while (Math.abs(randomY) > Sy); // Повторюємо генерацію, якщо число виходить за межі
